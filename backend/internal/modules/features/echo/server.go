...
import (
	...
	"waterfall-backend/internal/pkg/http"
	"waterfall-backend/internal/pkg/http/error_handler"
	"waterfall-backend/internal/pkg/http/middlewares"
	"waterfall-backend/internal/pkg/http/middlewares/server_time"
)
...
func newHTTPServer(config config.Config, lg *zap.Logger) *echo.Echo {
	...
	uses := []echo.MiddlewareFunc{
		...
		middlewares.RequestLogger(lg),
		middlewares.ContextLogger(lg),
