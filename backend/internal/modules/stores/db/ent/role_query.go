...
func (rq *RoleQuery) FirstID(ctx context.Context) (id roles.Type, err error) {
	var ids []roles.Type
...
func (rq *RoleQuery) FirstIDX(ctx context.Context) roles.Type {
...
func (rq *RoleQuery) OnlyID(ctx context.Context) (id roles.Type, err error) {
	var ids []roles.Type
...
func (rq *RoleQuery) OnlyIDX(ctx context.Context) roles.Type {
...
func (rq *RoleQuery) IDs(ctx context.Context) (ids []roles.Type, err error) {
...
func (rq *RoleQuery) IDsX(ctx context.Context) []roles.Type {
	ids, err := rq.IDs(ctx)
	if err != nil {
		panic(err)
	}
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				outValue := roles.Type(values[0].(*sql.NullString).String)
...
func (rq *RoleQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Role, init func(*Role), assign func(*Role, *Permission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[roles.Type]*Role)
	nids := make(map[permissions.Type]map[*Role]struct{})
	for i, node := range nodes {
		edgeIDs[i] = node.ID
		byID[node.ID] = node
		if init != nil {
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				outValue := roles.Type(values[0].(*sql.NullString).String)
				inValue := permissions.Type(values[1].(*sql.NullString).String)
				if nids[inValue] == nil {
					nids[inValue] = map[*Role]struct{}{byID[outValue]: {}}
					return assign(columns[1:], values[1:])
				}
