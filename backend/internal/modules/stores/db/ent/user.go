...
type User struct {
	...
	// VkID holds the value of the "vk_id" field.
	...
	SerialNumber uint `json:"serial_number,omitempty"`
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldVkID, user.FieldScore, user.FieldSerialNumber:
			values[i] = new(sql.NullInt64)
		case user.FieldID, user.FieldEmail, user.FieldFullname, user.FieldUsername, user.FieldPasswordHash:
	...
	for i := range columns {
		...
		case user.FieldVkID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field vk_id", values[i])
			} else if value.Valid {
				u.VkID = new(int64)
				*u.VkID = value.Int64
			}
		...
		case user.FieldSerialNumber:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				u.SerialNumber = uint(value.Int64)
			}
		default:
...
func (u *User) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", u.SerialNumber))
	builder.WriteByte(')')
