...
func (cq *CommentQuery) QueryChapter() *ChapterQuery {
...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withChapter:  cq.withChapter.Clone(),
...
func (cq *CommentQuery) WithChapter(opts ...func(*ChapterQuery)) *CommentQuery {
	query := (&ChapterClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withChapter = query
	return cq
}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withChapter; query != nil {
		if err := cq.loadChapter(ctx, query, nodes, nil,
			func(n *Comment, e *Chapter) { n.Edges.Chapter = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadAuthor(ctx context.Context, query *UserQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "author_uuid" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadParent(ctx context.Context, query *CommentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Comment)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_uuid" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CommentQuery) loadChildren(ctx context.Context, query *CommentQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Comment)) error {
	...
	for _, n := range neighbors {
		fk := n.ParentUUID
		if fk == nil {
			return fmt.Errorf(`foreign-key "parent_uuid" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "parent_uuid" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (cq *CommentQuery) loadChapter(ctx context.Context, query *ChapterQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Chapter)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Comment)
	for i := range nodes {
		if nodes[i].ChapterUUID == nil {
			continue
		}
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withAuthor != nil {
			_spec.Node.AddColumnOnce(comment.FieldAuthorUUID)
		}
		if cq.withParent != nil {
			_spec.Node.AddColumnOnce(comment.FieldParentUUID)
		}
		if cq.withChapter != nil {
			_spec.Node.AddColumnOnce(comment.FieldChapterUUID)
		}
	}
