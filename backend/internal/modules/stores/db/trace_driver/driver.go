...
type ILogger interface {
	...
	Trace(ctx context.Context, begin time.Time, fc func() (sql string), err error, data map[string]any)
}
...
func (d *TraceDriver) Exec(ctx context.Context, query string, args, v any) error {
	...
	d.logger.Trace(ctx, begin, f, err, nil)
...
func (d *TraceDriver) Tx(ctx context.Context) (dialect.Tx, error) {
	...
	d.logger.Debug(ctx, "Transaction started", map[string]any{"tx-id": id})
...
func (d *DebugTx) Exec(ctx context.Context, query string, args, v any) error {
	...
	d.logger.Trace(ctx, begin, f, err, map[string]any{"tx-id": d.id})
...
func (d *DebugTx) Commit() error {
	d.logger.Debug(context.Background(), "Transaction committed", map[string]any{"tx-id": d.id})
...
func (d *DebugTx) Rollback() error {
	d.logger.Debug(context.Background(), "Transaction rollbacked", map[string]any{"tx-id": d.id})
